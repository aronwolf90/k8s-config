...
- Set up a full working k8s in a few minutes.
...
This cluster is thought for personal projects and small
companies that need HA. This traduces in:
...
The module assumes that you have your public key in
`~/.ssh/id_rsa.pub`. Once you have adjusted your terraform
file, you just have to run `tofu apply`.
...
- 3 controller+worker nodes (CX21):
  `3 * 2CPU and 3 * 4GB -> 3 * 5,83 EUR = 16,49 EUR`
- Load balancer (LB11) -> 5,83 EUR

This is at the moment of writing 22,32 EUR per month.

NOTE: Comparing to GCloud, it is 3 times cheaper for the first cluster and 5 times
cheaper for the second cluster (For the first cluster GCloud does not charge for
the master node).
...
- Execute:
  ...
  - For testing all supported K8s versions:
    `cd tests/e2e && TEST_K8S_VERSIONS=true go test -timeout 99999s`
