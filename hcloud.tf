...
resource "hcloud_load_balancer" "master" {
  ...
  depends_on         = [hcloud_server.master, hcloud_server.master2]
  location           = "nbg1"
}
...
resource "hcloud_load_balancer_target" "master2" {
  type             = "server"
  load_balancer_id = hcloud_load_balancer.master.id
...
resource "null_resource" "install_kubernetes" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "kubeadm init --control-plane-endpoint ${hcloud_load_balancer.master.ipv4}:6443 --upload-certs --ignore-preflight-errors=DirAvailable--var-lib-etcd --pod-network-cidr=10.244.0.0/16",
...
resource "null_resource" "install_master2" {
  depends_on = [
  ...
  triggers = {
    hcloud_server_master_id = hcloud_server.master2.id
  }

  connection {
    host        = hcloud_server.master2.ipv4_address
    type        = "ssh"
    user        = "root"
    private_key = file(var.private_key)
  }
  ...
  provisioner "remote-exec" {
    inline = [
      "bash /tmp/install_kubedm.rb",
      "echo ${data.external.kubeadm_join_master.result.command}",
      data.external.kubeadm_join_master.result.command
    ]
  }
}
...
data "external" "kubeadm_join_master" {
  program = ["./kubeadm_master_token.sh"]
